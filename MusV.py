...
def buttons(message):
    ...
    #markup.add(btn1)
    markup.add(btn2)
    bot.send_message(message.chat.id,"Привет я бот что бы искать и скачивать треки",reply_markup=markup)
...
def bot_butn_clck(call):
    ...
    if call.data=="/next":
        ...
        try:
            ...
            print(tmp)
            filee=open(tmp,'rb')
            bot.send_message(call.message.chat.id,"Грузим ваш файл")
            bot.send_audio(call.message.chat.id,filee,timeout=500)
            os.remove(tmp)
            filee.close()
        except IndexError:
            bot.send_message(call.message.chat.id,"Выполните запрос")
